  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-card overflow-hidden">
      <div className="flex flex-col lg:flex-row lg:items-stretch">
        {/* ── Sol sütun: takvim (kompakt, sabit genişlik) ── */}
        <div className="lg:w-[360px] lg:flex-shrink-0 lg:border-r lg:border-slate-100">
          {/* Başlık */}
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-sm font-semibold text-slate-700 px-1 min-w-[104px] text-center">
                {AY_ADLARI[ay]} {yil}
              </span>
              <button
                type="button"
          ...
          <div className="p-4">
            ...
            <div className="grid grid-cols-7 mb-1">
              {GUN_ADLARI.map((g) => (
                <div key={g} className="text-center text-xs font-medium text-slate-400 py-1">
                  {g}
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-7 gap-y-0.5">
              {gunler.map((gun, idx) => {
                ...
                // Günün baskın rengi (ilk olay belirler) — açık tonlu arka plan
                ...
                return (
                  <button
                    ...
                    className={[
                      "relative flex flex-col items-center py-0.5 rounded-lg transition-colors cursor-pointer",
                      isSelected ? "bg-blue-50 ring-1 ring-blue-200" : hasOlay ? `${gunBg} hover:brightness-95` : "hover:bg-slate-50",
                    ].join(" ")}
                  >
                    <span
                      className={[
                        "text-xs w-8 h-8 flex items-center justify-center rounded-full font-medium transition-colors",
                        isToday
                    ...
                    {noktalar.length > 0 && (
                      <div className="flex gap-0.5 mt-0.5 h-1.5 items-center">
                        {noktalar.map((o, i) => (
                          <span key={i} className={`w-1 h-1 rounded-full ${RENK_DOT[o.renk]}`} />
                        ))}
                      </div>
                    )}
                  </button>
            ...
            <div className="mt-3 pt-3 border-t border-slate-100 flex flex-wrap gap-x-3 gap-y-1.5">
              <span className="flex items-center gap-1.5 text-xs text-slate-500">
                <span className="w-2 h-2 rounded-full bg-red-500 flex-shrink-0" /> Beyanname
              </span>
              <span className="flex items-center gap-1.5 text-xs text-slate-500">
                <span className="w-2 h-2 rounded-full bg-blue-500 flex-shrink-0" /> Görev
              </span>
              <span className="flex items-center gap-1.5 text-xs text-slate-500">
                <span className="w-2 h-2 rounded-full bg-amber-500 flex-shrink-0" /> Tahsilat
              </span>
              <span className="flex items-center gap-1.5 text-xs text-slate-500">
        ...
        {/* ── Sağ sütun: seçilen günün ajandası (kalan genişliği doldurur) ── */}
        <div
          className={[
            "lg:flex-1 lg:min-w-0 bg-slate-50/60 border-t border-slate-100 lg:border-t-0",
            secilenGun === null ? "hidden lg:block" : "",
          ].join(" ")}
        >
          {secilenGun === null ? (
            <div className="hidden lg:flex h-full items-center justify-center p-6 text-center">
              <p className="text-xs text-slate-400">Ayrıntıları görmek için bir gün seçin.</p>
            </div>
          ) : (
            <>
              ...
              {secilenOlaylar.length === 0 ? (
                <p className="px-5 py-4 text-xs text-slate-400">Bu gün için etkinlik yok.</p>
              ) : (
                <div className="divide-y divide-slate-100 pb-2 lg:max-h-[300px] lg:overflow-y-auto">

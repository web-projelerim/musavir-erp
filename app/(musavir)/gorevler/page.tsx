...
export default function GorevlerPage() {
  ...
  const [gorevler, setGorevler] = useState(MOCK_GOREVLER);

  const handleDurumGuncelle = (id: string, durum: GorevDurum) => {
    setGorevler((prev) =>
      prev.map((g) => (g.id === id ? { ...g, durum } : g))
    );
  };
  ...
  const bekleyenSayi = gorevler.filter((g) => g.durum !== "tamamlandi" && g.durum !== "iptal").length;
